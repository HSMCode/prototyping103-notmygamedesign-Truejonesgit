...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            {
                target.transform.position += new Vector3(0, 1f, 0); //if we press w add 1 to the y value (up)
            }

            if (Input.GetKeyDown(KeyCode.A))
            {
                target.transform.position += new Vector3(-1f, 0, 0); //if we press a add -1 to the x value (left)
                defaultSprite.flipX = false; //to flip the sprite on x axis
            }

            if (Input.GetKeyDown(KeyCode.S))
            {
                target.transform.position += new Vector3(0, -1f, 0); //if we press s add -1 to the y value (down)
            }

            if (Input.GetKeyDown(KeyCode.D))
            {
                target.transform.position += new Vector3(1f, 0, 0); //if we press d add 1 to the x value (right)
